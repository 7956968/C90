...
INT32 SetupExe_OnScreenLight(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    UINT32 uhSelect = 0;
    if (paramNum > 0) {
        uhSelect = paramArray[0];
    ...
    UI_SetData(FL_SCREENLIGHT, uhSelect);


    return NVTEVT_CONSUME;
}

INT32 SetupExe_OnAlarm_LCA(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    UINT32 uhSelect = 0;
    if (paramNum > 0) {
        uhSelect = paramArray[0];
    ...
    UI_SetData(FL_ALARM_LCA, uhSelect);
...
//#MT#lyb -20200328 -end
#if(_ANFA_ADAS_MENU_)
INT32 SetupExe_OnANFA_ADAS(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    UINT32 uhSelect = 0;
    if (paramNum > 0) {
        uhSelect = paramArray[0];
...
#if(WIFI_FUNC==ENABLE)
UINT32 g_PrevWifiMode;
void Sys_SetPrevWifiMode(UINT32 mode)
{
	if (mode < NET_MODE_SETTING_MAX)
		g_PrevWifiMode = mode;
	else
		g_PrevWifiMode = NET_AP_MODE;
}

UINT32 Sys_GetPrevWifiMode(void)
{
	return g_PrevWifiMode;
}
...
INT32 SetupExe_OnWifiMode(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	Sys_SetPrevWifiMode(UI_GetData(FL_NetWorkMode)); // backup last wifi mode
...
INT32 SetupExe_OnWifiStart(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	debug_msg("SetupExe_OnWifiStart  FL_WIFI:%d \r\n", UI_GetData(FL_WIFI));
	//if (UI_GetData(FL_WIFI) == WIFI_OFF)
	//{
	//	return NVTEVT_CONSUME;
	//}
...
INT32 SetupExe_OnWifiStop(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	if ((ImageApp_MovieMulti_IsStreamRunning(_CFG_REC_ID_1) == FALSE) && (ImageApp_MovieMulti_IsStreamRunning(_CFG_REC_ID_1) == FALSE)){
	#else
	{
	#endif
...
//#MT#lyb -20200328 -begin
    {NVTEVT_EXE_SCREENLIGHT,            SetupExe_OnScreenLight      },
    {NVTEVT_EXE_ALARM_LCA,              SetupExe_OnAlarm_LCA        },
//#MT#lyb -20200328 -end
#if(_ANFA_ADAS_MENU_)
    {NVTEVT_EXE_ANFA_ADAS,              SetupExe_OnANFA_ADAS        },
#endif
