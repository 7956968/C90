...
NVT_VB_CPOOL_S g_movie_common_pool_tbl[NVTMPP_VB_MAX_COMM_POOLS+1]= {
	...
	{ 1280 *  320 * 3 / 2 + 1024, 5, NVTMPP_DDR_1 }, // display					size = 0x10E400--------1
	...
	#endif
	{  ALIGN_CEIL_64(1280) *  ALIGN_CEIL_64(720) * 3 / 2 *4 + VDODEC_H264_CODEC_SIZE+ 1024 , 1, NVTMPP_DDR_1 }, // for VdoDec	size = 0XC80400------3
#endif
	#if (_ANFA_ADAS_FUNC_ == ENABLE)
	{  640 *  360 * 3 / 2 + 1024, 4, NVTMPP_DDR_1 }, // 2 wifi					size =  --------4
	#else
	...
	{  848 *  480 * 3 / 2 + 1024, 3, NVTMPP_DDR_1 }, // 2 ALG					size = 0X95500---------5
	#endif
