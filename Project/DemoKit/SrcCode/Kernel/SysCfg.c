...
#if (IPCAM_FUNC !=ENABLE) && defined(_NETWORK_ON_CPU2_)
#include "WiFiIpcAPI.h"
#endif
...
UINT32 POOL_ID_WAV_PLAY_HEADER = 0;
UINT32 POOL_ID_WAV_PLAY_DATA = 0;
UINT32 POOL_ID_USER_PARTITION_RW_BUFFER = 0;
...
//for EthCamSockIpc
#if (defined(_NVT_ETHREARCAM_RX_))
UINT32 POOL_ID_ETHSOCKCLI_IPC = 0;
UINT32 POOL_ID_ETHUDPSOCK_IPC = 0;
#endif
#if (defined(_NVT_ETHREARCAM_TX_))
UINT32 POOL_ID_ETHSOCK_IPC = 0;
#endif
...
//Lib
	...
	UVAC_InstallID();
	ISF_UsbUVAC_InstallID();
	ImageApp_UsbMovie_InstallID();
#endif
#if (HTTP_LIVEVIEW_FUNC)
	LviewNvt_InstallID();
#endif
...
//for fileDB
	...
	OS_CONFIG_MEMPOOL(POOL_ID_WAV_PLAY_HEADER,	POOL_SIZE_WAV_PLAY_HEADER,	POOL_CNT_WAV_PLAY_HEADER);
	OS_CONFIG_MEMPOOL(POOL_ID_WAV_PLAY_DATA,	POOL_SIZE_WAV_PLAY_DATA,	POOL_CNT_WAV_PLAY_DATA);
#endif

#if (USER_PACK_UPDATE == ENABLE)
    OS_CONFIG_MEMPOOL(POOL_ID_USER_PARTITION_RW_BUFFER,  POOL_SIZE_USER_PARTITION_RW_BUFFER,    POOL_CNT_USER_PARTITION_RW_BUFFER);
#endif
}
...
#if (defined(_NVT_ETHREARCAM_RX_))
OS_CONFIG_MEMPOOL(POOL_ID_ETHSOCKCLI_IPC,     	POOL_SIZE_ETHSOCKCLI_IPC,        	POOL_CNT_ETHSOCKCLI_IPC);      // EthSockCliIPc working buffer#else
#if defined(_CPU2_LINUX_)
OS_CONFIG_MEMPOOL(POOL_ID_ETHUDPSOCK_IPC,     	POOL_SIZE_ETHUDPSOCK_IPC,        	POOL_CNT_ETHUDPSOCK_IPC);      // EthUDPSockCli working buffer#else
#endif
#endif
#if (defined(_NVT_ETHREARCAM_TX_))
OS_CONFIG_MEMPOOL(POOL_ID_ETHSOCK_IPC,     		POOL_SIZE_ETHSOCK_IPC,        		POOL_CNT_ETHSOCK_IPC);          // EthSockIPc working buffer
#endif
}
