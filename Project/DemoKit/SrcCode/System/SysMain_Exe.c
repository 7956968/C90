...
#endif
extern void  System_OnStrg_TxFirmware(void);
...
//CHKPNT;
		...
		//EthCamNet_SetTxIPAddr();//after load pstore  move cj 0325
...
//			);
		...
		if (System_GetState(SYS_STATE_POWERON) == SYSTEM_POWERON_SAFE ||System_GetState(SYS_STATE_POWERON) == SYSTEM_POWERON_NORMAL ) {
		//	CHKPNT;
...
//CHKPNT;
...
#if ((QH_CJSON_FUNCTION == ENABLE)||(_ALPHAVISION_LICENSE_ == ENABLE))
       // App_cJSON_Init();
#endif
	...
	if ( fristIn) 
	{   
		GxSound_SetVolume(Get_VolumeValue(SysGetFlag(FL_BeepVolumnIndex)));
	...
	//debug_msg("System_OnBoot  FL_WIFI:%d \r\n", UI_GetData(FL_WIFI));
if (UI_GetData(FL_WIFI) == WIFI_ON &&WIFI_FUNC ==ENABLE)
	{
	  	System_WifiStart();//add start wifi, make sure wifi power and CS  gpio high
	}
...
//#MT#lyb -20200410 -begin
    if(System_GetState(SYS_STATE_CURRMODE) == PRIMARY_MODE_MOVIE)
    {
        if(UI_GetData(FL_MOVIE_REC))
        {
