...
//#MT#lyb -20200403 -begin
    if(GxSystem_GetPowerOnSource() == GX_PWRON_SRC_PWR_VBAT) 
    {
		...
		//add  cj 2020 0612 
		//if ( rtc_getPWR2Status()  ||( !rtc_getPWR2Status() && UI_GetData(FL_MOVIE_PARK) == MOVIE_PARK_OFF ) )
		//{
		//	rtc_resetShutdownTimer();
		//	debug_msg("cj-------ACC =  %d FL_MOVIE_PARK = %d,so poweroff\r\n",rtc_getPWR2Status(), UI_GetData(FL_MOVIE_PARK) );
		//	rtc_poweroffPWR();
		//	while(1);
		//}
    } 
