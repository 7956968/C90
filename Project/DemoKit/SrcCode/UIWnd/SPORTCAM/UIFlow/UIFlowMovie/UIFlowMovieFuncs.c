...
BOOL FlowMovie_IsStorageErr(BOOL IsCheckFull)
{
	...
	if (UIStorageCheck(STORAGE_CHECK_ERROR, NULL) == TRUE) {
		Ux_OpenWindow(&UIFlowWndWrnMsgCtrl, 2, FLOWWRNMSG_ISSUE_CARD_ERR, FLOWWRNMSG_TIMER_KEEP);
		return TRUE;
	}
...
void FlowMovie_OnHideCtrl(VControl *pCtrl)
{
	UxCtrl_SetShow(pCtrl, FALSE);
}

static UINT32  g_CurrTime = 0;
static CHAR    g_EMRCurrTimeStr[20] = {0};
extern  UINT32	g_EmergencyRecSetp ;
extern UINT32   g_EmergencyRecTime ;
extern UINT32   g_EmergencyRecSetpTotail ;
...
//#MT#lyb -20200317 -begin
   // /*
		if (UxCtrl_IsShow(&UIFlowWndMovie_Panel_DatetimeCtrl)) {
			FlowMovie_IconDrawDateTime();
		}
  //  */
//#MT#lyb -20200317 -end

	//if ( g_bRecordLock  == TRUE)
	if ( UIFlowWndMovie_GetRecLockStatus() )
	{
		...
		UxCtrl_SetShow(&UIFlowWndMovie_Status_Movie_FCtrl, FALSE);
		UxCtrl_SetShow(&UIFlowWndMovie_Status_Movie_BCtrl, FALSE);
		...
		UxStatic_SetData(&UIFlowWndMovie_Static_RecUrgTxtCtrl,STATIC_VALUE,STRID_RECORDING_URG);
		UxCtrl_SetShow(&UIFlowWndMovie_Static_RecUrgTxtCtrl, TRUE);
		UxCtrl_SetShow(&UIFlowWndMovie_Panel_RecUrgCtrl, TRUE);
	
    	g_EmergencyRecTime ++;
    //debug_err(("EmergencyRec stp= %d \r\n",g_EmergencyRecTime));
    //debug_err((" EmergencyRec tot = %d\r\n",g_EmergencyRecSetpTotail));
		g_CurrTime = FlowMovie_GetRecCurrTime() ;
    	if (g_EmergencyRecTime <= g_CurrTime)
        {
        // 紧急录像取消标志?
            if (g_EmergencyRecTime > 10)
            {
               // g_CancelEmergency = TRUE;
            }
			memset((void *)g_EMRCurrTimeStr, 0, sizeof(g_EMRCurrTimeStr));
			snprintf(g_EMRCurrTimeStr, 20, "%02d:%02d:%02d", g_CurrTime / 3600, (g_CurrTime % 3600) / 60, (g_CurrTime % 3600) % 60);

       	 	g_EmergencyRecSetp  = g_CurrTime; //(g_CurrTime % 3600) % 60;// g_EmergencyRecTime;
       	 	if(System_GetGsensorPwrOn()){
				//CHKPNT;
				//g_EmergencyRecSetp	= g_CurrTime+1; //(g_CurrTime % 3600) % 60;// g_EmergencyRecTime;
			}
        	UxProgressBar_SetData(&UIFlowWndMovie_ProgressBarCtrl, PROBAR_TOTSTP, g_EmergencyRecSetpTotail);
        	UxProgressBar_SetData(&UIFlowWndMovie_ProgressBarCtrl, PROBAR_CURSTP, g_EmergencyRecSetp);
			UxStatic_SetData(&UIFlowWndMovie_Static_ENRTimeCtrl, STATIC_VALUE, Txt_Pointer(g_EMRCurrTimeStr));
			
			//DBGD(g_EmergencyRecSetpTotail);
			//debug_msg("PROBAR_CURSTP: %d\r\n",UxProgressBar_GetData(&UIFlowWndMovie_ProgressBarCtrl, PROBAR_CURSTP) );

			if(UxProgressBar_GetData(&UIFlowWndMovie_ProgressBarCtrl, PROBAR_CURSTP) == 0)
			{
			...
			UxCtrl_SetShow(&UIFlowWndMovie_Panel_EMRBarCtrl, TRUE);
        	UxCtrl_SetShow(&UIFlowWndMovie_ProgressBarCtrl, TRUE);
			UxCtrl_SetShow(&UIFlowWndMovie_Static_ENRTimeCtrl, TRUE);
        }else{
		...
		if (UxCtrl_IsShow(&UIFlowWndMovie_ConfirmPanelCtrl))
		{
