...
void DxVoiceCtrl_AddSinglePlayQueue(UINT8 pBuf)
{
    UINT32 tempbuf = 0;
    tempbuf = pBuf|0xFF00;

  //  DxVoiceCtrl_enQueue(&PlayVoiceInfo, tempbuf);

    //DBG_DUMP("adds in %d: 0x%04x ,0x%x ",PlayVoiceInfo.writep,PlayVoiceInfo.voice_array_int[PlayVoiceInfo.writep],pBuf);
}
void UISound_Play(UINT32 index)
{
	...
	//debug_msg("UI_GetData(FL_BEEP) :%d  %d\r\n",UI_GetData(FL_BEEP),index );
	#if (AUDIO_FUNC == ENABLE)
...
#endif
//DxVoiceCtrl_AddSinglePlayQueue();
		GxSound_Play(index);
	}
	#endif
}
