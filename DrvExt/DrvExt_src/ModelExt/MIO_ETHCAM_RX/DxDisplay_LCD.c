...
#define DRVLCD_BRT_LEVEL_INIT    99
...
static void GPIOMap_SetLCDBacklightBrightLevelSeamless(INT32 uiLevel);
//dx object
DX_OBJECT gDevLCD = {
	DXFLAG_SIGN,
...
static UINT32 DrvLCDGetcaps(UINT32 CapID, UINT32 Param1) // Get Capability Flag (Base on interface version)
{
	...
	case DISPLAY_CAPS_MAXSIZE: {
			...
			if (pSizeInfo) {
				...
				DBG_DUMP("^C DISPLAY_CAPS_MAXSIZE: w %d, h %d\r\n", pSizeInfo->w, pSizeInfo->h);
			}
		}
		break;
...
//CHKPNT;
	...
	UINT32 PAD_DRIVINGSINK_VALUE =0;
	DrvLCD_TurnOn();
	pad_getDrivingSink(PAD_DS_LCD8, &PAD_DRIVINGSINK_VALUE);
	debug_msg("PAD_DRIVINGSINK_VALUE ；0x%x \r\n ", PAD_DRIVINGSINK_VALUE);
	...
	if (StateID & DXGET) {
		...
		case DRVDISP_STATE_BACKLIGHT:
			if (Value == DRVDISP_BACKLIGHT_OFF) {
				...
				//DBG_DUMP("DRVDISP_BACKLIGHT_ON\r\n");
				//DBGD(g_EnterParkingMode_CloseBL);
				//if(g_EnterParkingMode_CloseBL == FALSE)	//zjf 12-15 
					GPIOMap_TurnOnLCDBacklight();
			}
			break;
		case DRVDISP_STATE_BRIGHTLVL: {
			...
			break;
		case DRVDISP_STATE_BRIGHTLVL_SEAMLESS:
...
static PWM_CFG g_LCDBacklightPWMInfo = {100, 0, 100, 0, 1};
...
#if (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)
		if(g_EnterParkingMode_CloseBL == TRUE){
			CHKPNT;
			g_LCDBacklightPWMInfo.uiRise = 0;
		}else{
			g_LCDBacklightPWMInfo.uiRise =  g_LCDBacklightLvl;
		}
		DBG_IND("PWM: rise=%d\r\n", g_LCDBacklightPWMInfo.uiRise);
		//gpio_setPin(GPIO_BL_EN);
		pwm_open(PWMID_LCD_BLG_PCTL);
		//UINT32    clk_div = 120000000 / 100 / 30000 - 1;    // = 19

		//pwm_pwmConfigClockDiv(PWM4_7_CLKDIV,clk_div); //PWM0_3_CLKDIV,1999
				pwm_pwmConfigClockDiv(PWM4_7_CLKDIV,49); //PWM0_3_CLKDIV,1999
			//	pwm_pwmConfigClockDiv(PWM8_11_CLKDIV,1999);  //cj 0716
...
static void GPIOMap_SetLCDBacklightBrightLevel(INT32 uiLevel)
{
	...
	#if 0
	if (g_LCDBacklightLvl < 40) {
		CHKPNT;
		DBG_WRN("DrvLCDSetCtrl Warning! brightlvl=0x%02X out of range\r\n", g_LCDBacklightLvl);

		g_LCDBacklightLvl = 40;
		#if 0
		if  (uiLevel == 0 )
		{
			g_LCDBacklightLvl = 40;
		}else{
			g_LCDBacklightLvl = uiLevel +40;
		}
		#endif
	}
	...
	#endif
	g_LCDBacklightLvl = 4+uiLevel*(100-4)/100.0; //zjf add --- 保持和自动背光调节一样的范围 22-99
	//debug_msg("uiLevel:%d \r\n",uiLevel);
...
static void GPIOMap_AdjustLCDBacklight(UINT32 uiAdjValue)
{
	...
	if (g_LCDBacklightEn) {
		CHKPNT;
#if (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)
		if  (g_LCDBacklightLvl < 2)
		{
			g_LCDBacklightLvl = 1 ;
		}
	
		g_LCDBacklightPWMInfo.uiRise = g_LCDBacklightLvl;
		//g_LCDBacklightPWMInfo.uiOnCycle = 100;
		//g_LCDBacklightPWMInfo.uiFall = 100;

		debug_msg("MMMMM PWM: cycle=%d, %d ,%d\r\n",g_LCDBacklightPWMInfo.uiPrd, g_LCDBacklightPWMInfo.uiRise,  g_LCDBacklightPWMInfo.uiFall
			);
		pwm_pwmConfig(PWMID_LCD_BLG_PCTL, &g_LCDBacklightPWMInfo);
...
#endif
UINT32   g_LCDBacklightLvlSeamless =0;

void GPIOMap_SetLCDBacklightBrightLevelSeamless(INT32 uiLevel)
{
	//static UINT8 debug_msg_cnt = 0;
#if (LCD_BACKLIGHT_CTRL == LCD_BACKLIGHT_BY_PWM)

    g_LCDBacklightLvlSeamless = uiLevel;
//debug_msg("uiLevel:%d \r\n", uiLevel);
    if(g_LCDBacklightLvlSeamless<2)
    {
        g_LCDBacklightLvlSeamless = 1;
    }
    if(g_LCDBacklightLvlSeamless >= 100)
    {
        g_LCDBacklightLvlSeamless = 99;
    }

    if(!g_LCDBacklightEn)
        return;
	
	g_LCDBacklightPWMInfo.uiRise = g_LCDBacklightLvlSeamless; //(100 - g_LCDBacklightLvlSeamless);
	DBG_IND("PWM: rise=%d\r\n", g_LCDBacklightPWMInfo.uiRise);
	
	//debug_msg_cnt++;
	//if(debug_msg_cnt == 10){
	//	debug_msg("MMMMM LevelSeamless: cycle=%d, %d ,%d\r\n",g_LCDBacklightPWMInfo.uiPrd, 
	//   	g_LCDBacklightPWMInfo.uiRise,  g_LCDBacklightPWMInfo.uiFall);
	//	debug_msg_cnt = 0;
	//}
